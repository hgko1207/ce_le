...
ce90_circle = plt.Circle((np.min(results), np.min(results)), CE90, fill=False, color='green', label='CE90')
# ce95_circle = plt.Circle((np.mean(xValues), np.mean(yValues)), CE95, fill=False, label='CE95')
axes.set(xlim=(-2, 10), ylim = (-5, 6))
axes.set_aspect(1)
# axes.add_artist(ce50_circle)
axes.add_artist(ce90_circle)
# axes.add_artist(ce95_circle)

plt.scatter(x=xValues, y=yValues, s=0.5, c='blue', label='errors')
plt.scatter(x=results, y=results, s=0.5, c='red', label='errors')
